...
weighted_tree::weighted_tree(size_t size)
    : size(size), rootIdx(0), vertices(std::vector<tree_vertice>(size)) {
    for (int i = 0; i < size; ++i) vertices[i].idx = i;
}
...
void weighted_tree::run_preorder(int idx, std::function<void(tree_vertice&)>& func) {
    func(vertices[idx]);
    for (auto& edge : vertices[idx].children) run_preorder(edge.destIdx, func);
}
...
void weighted_tree::run_postorder(int idx, std::function<void(tree_vertice&)>& func) {
    for (auto& edge : vertices[idx].children) run_postorder(edge.destIdx, func);
    func(vertices[idx]);
}
