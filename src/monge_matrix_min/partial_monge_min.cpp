...
int recursive_partial_monge_min(const size_t rowBegin, const size_t colBegin, 
                                const size_t size, std::function<int(size_t, size_t)> lookup);

int monge_sub_sqr_min(const size_t rowBegin, const size_t colBegin, 
                      const size_t size, std::function<int(size_t, size_t)> lookup);
...
int recursive_partial_monge_min(const size_t rowBegin, const size_t colBegin, 
    ...
    if (prefSize > 0) {
        result = std::min(
            result, 
            recursive_partial_monge_min(rowBegin, colBegin, prefSize, lookup)
        );
        result = std::min(
            result,
...
int monge_sub_sqr_min(const size_t rowBegin, const size_t colBegin, 
                      const size_t size, std::function<int(size_t, size_t)> lookup) {
    return smawk_min(size, size, [&](size_t i, size_t j) -> int {
        return lookup(rowBegin + i, colBegin + j);
    });
}