    ...
    for (auto & pair : hld.interesting_pairs) {
        for (auto & eP : pair.edgesP) {
            for (auto & eQ : pair.edgesQ)
                found_interested[eP.srcIdx][eQ.srcIdx] = found_interested[eQ.srcIdx][eP.srcIdx] = true;
        }
    }
    ...
    for (auto & v1 : tree.vertices) {
        for (auto v2 : tree.vertices) {
            if (v1.idx == tree.rootIdx || v2.idx == tree.rootIdx || v1.hlPathId == v2.hlPathId)
                continue;
            if (gmw.is_crossinterested(v1.parentEdge, v2.parentEdge) && gmw.is_crossinterested(v2.parentEdge, v1.parentEdge)) {
                ASSERT_TRUE(found_interested[v1.idx][v2.idx]) 
                    << "edges: " << v1.parentEdge << "; " << v2.parentEdge 
                    << "are mutually CROSS-interested and should be included in interesting pairs for graph:\n" << graph 
                    << "\n and spanning tree: \n" << tree;
            }
            if (gmw.is_downinterested(v1.parentEdge, v2.parentEdge) && gmw.is_downinterested(v2.parentEdge, v1.parentEdge)) {
                ASSERT_TRUE(found_interested[v1.idx][v2.idx]) 
                    << "edges: " << v1.parentEdge << "; " << v2.parentEdge 
                    << "are mutually DOWN-interested and should be included in interesting pairs for graph:\n" << graph 
                    << "\n and spanning tree: \n" << tree;
            }
        }
    }
...
void test_number_of_interesting_pairs(graphs::weighted_graph & graph, graphs::WeightedTree & tree, graphs::hl_decomposition & hld) {
    ...
    EXPECT_LE(total_pairs, upper_bound) << "number of interesting pairs should be less than " << upper_bound
        << "and is " << total_pairs
        << "for graph: \n" << graph 
        << "and spanning tree: \n" << tree;
}
...
TEST(Graphs_HLDecomposition, HLDRandomTest) {
    ...
    while (testCases--)
        hld_test_random_graph(distN(*seed), maxWeight, seed);
}
