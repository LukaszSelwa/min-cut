...
class TrivialRangeSearch : public RangeSearchStructure {
   ...
   public:
    ...
    void AddPoint(int x, int y, int val) override { arr[x][y] += val; }
...
TEST(RangeSearch_Interval2DTree, GetSumInRectangleTest) {
    ...
    EXPECT_EQ(search->GetSumInRectangle(1, 9, 1, 8), 12)
        << "Rectangle [1,9]x[1, 8] should have value 12";
}
...
void RunSingleRandomTest(int size, int adds, int sums, std::mt19937& seed) {
    ...
    for (int s = 0; s < sums; s++) {
        ...
        EXPECT_EQ(tst->GetSumInRectangle(x0, x1, y0, y1), ver->GetSumInRectangle(x0, x1, y0, y1))
            << "RANDOM_TEST";
    }
}
