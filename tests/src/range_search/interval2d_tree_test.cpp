...
class TrivialRangeSearch : public range_2d {
   ...
   public:
    ...
    void add_point(int x, int y, int val) override { arr[x][y] += val; }
...
void RunSingleRandomTest(int size, int adds, int sums, std::mt19937& seed) {
    ...
    std::unique_ptr<range_2d> ver = std::make_unique<TrivialRangeSearch>(size, size);
