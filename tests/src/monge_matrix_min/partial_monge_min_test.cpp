...
TEST(MongeMatrixMin_PartialMonge, SmallExampleTest_1) {
    ...
    std::function<int(size_t, size_t)>lookup = [&](size_t row, size_t col) {
        if (row < col) {
            std::stringstream ss;
            ss << "should only lookup in lower trinagle but looked-up (" 
               << row << ", " << col << ")";
...
TEST(MongeMatrixMin_PartialMonge, mediumExampleTest) {
    int mongeArr[9][9] = {
        {  25,  -1,  -1,  -1,  -1,  -1,  -1,  -1,  -1},
        {  42,  35,  -1,  -1,  -1,  -1,  -1,  -1,  -1},
        {  57,  48,  35,  -1,  -1,  -1,  -1,  -1,  -1},
        {  78,  65,  51,  42,  -1,  -1,  -1,  -1,  -1},
        {  90,  76,  58,  48,  49,  -1,  -1,  -1,  -1},
        { 103,  85,  67,  56,  55,  44,  -1,  -1,  -1},
        { 123, 105,  86,  75,  73,  59,  57,  -1,  -1},
        { 142, 123, 100,  86,  82,  65,  61,  62,  -1},
...
void test_random_partial_monge_matrix(const size_t maxSize,
                              const int initBound,
                              const int maxInc,
                              std::mt19937 seed) {
...
TEST(MongeMatrixMin_PartialMonge, RandomSmallTest) {
    ...
    std::mt19937 seed(rd());
    while (testCases--)
        test_random_partial_monge_matrix(maxSize, initBound, maxInc, seed);
}
